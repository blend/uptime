...
Monitor.prototype.pollChecksNeedingPoll = function(callback) {
  ...
  this.findChecksNeedingPoll(function(err, checks) {
    ...
    checks.forEach(function(check) {
      self.pollCheck(check, self.config.retries, function(err) {
 ...
 * @api   private
 */
Monitor.prototype.pollCheck = function(check, retriesLeft, callback) {
  if (!check) return;
  var Poller, p;
  var now = Date.now();
  var self = this;
  ...
  var pollerCallback = function(err, time, res, pollerDetails) {
    if (err) {
      if (retriesLeft > 1) {
        // retry the poll in 5 seconds if we have retries left
        setTimeout(function(){ self.pollCheck(check, retriesLeft - 1, callback); }, 5000);
        return;
      } else {
